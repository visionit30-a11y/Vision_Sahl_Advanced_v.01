...
**الحالة:** خطة قبول مقترحة في G1 بتاريخ 2026-09-08؛ لا implementation gate جديدة PASS.
**الأساس:** `phase-2d-baseline` — `1677d114c5736c4032b862ddf0b58517490d1317`.
**Migration الحالية:** `0013_auth_http_projections`؛ لا migration في G1.

المرجع: [ADR-0025](adr/ADR-0025-security-audit-and-hardening.md) و[خطة Phase 2E](phase-2e.md).

## مصفوفة القبول النهائي

| # | معيار القبول | الإثبات المطلوب لاحقًا | حالة G1 |
| --- | --- | --- | --- |
| 1 | كتالوج event/result/reason مركزي typed وثابت | unknown/duplicate/mapping tests وDB catalog equality | PLANNED |
| 2 | اكتمال emitters للعمليات الأمنية القائمة | coverage لكل حدث، بلا endpoints مصطنعة أو auth redesign | PLANNED |
| 3 | IDs وروابط موثوقة فقط | forged user/session/membership/role IDs وcross-tenant denial | PLANNED |
| 4 | حصر الحقول ومنع الأسرار | canaries لـpassword/PHC/bearer/reset/CSRF/keys/DSNs، ورفض raw metadata | PLANNED |
| 5 | تغيير الحالة والحدث ذريّان | فشل event يلغي mutation؛ retry/no-op لا يخلق نجاحًا مضللًا | PLANNED |
| 6 | failure/denial لا تختفي بصمت أو تفتح الوصول | rollback + bounded audit transaction، DB outage يبقى DENY | PLANNED |
| 7 | runtime append-only وأقل grants | لا direct SELECT/INSERT/UPDATE/DELETE/TRUNCATE أو ownership؛ exact function guards | PLANNED |
| 8 | تصنيف DB والعزل ثابتان | auth exception محددة، جميع tenant RLS/FORCE/USING/WITH CHECK guards | PLANNED |
...
| 13 | metadata وcontext آمنان | route template، internal correlation، request-ID compatibility وfinally/concurrency | PLANNED |
...
| 21 | regressions وكامل quality/database gates | Backend/Frontend كاملة، Ruff/Mypy/ESLint/TS/Prettier/build، Alembic/round-trip/RLS/cleanup | PLANNED |
...
**النتيجة الحالية:** 22 معيارًا مخططًا؛ لم تُشغّل ولم تُقبل بوابات Phase 2E بعد.
G1 design review ليست بديلًا عن هذه الإثباتات. لا نتائج scanner أو benchmark أو test counts جديدة.
